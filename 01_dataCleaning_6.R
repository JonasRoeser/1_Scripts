...
D1 = D1[c(1:4,19,37,55)] %>%
  mutate(form_player0 = NA) %>%
  mutate(form_player1 = NA)

D1 = D1%>%
  mutate(mcFreezy = paste(D1$player0, D1$player1, D1$player0, sep = "-"))

for (i in 92000:nrow(D1)) {
  ...
  if (n_matches >= 10) {
    sth = D1$Y[matches_smaller_iboth[c((n_matches)-9):n_matches]]
    ...
    D1$form_player1[i] = length(which(sth[sth0] == 0)) + length(which(sth[sth1] == 1))
  } else {
  ...
  if (n_matches >= 10) {
    ...
    bruv1 = match(matches_smaller_i1[c((n_matches)-9):n_matches], matches_smaller_iboth[c((n_matches)-9):n_matches])
    sth1 = bruv1[which(!is.na(bruv1))]
    D1$form_player0[i] = length(which(sth[sth0] == 0)) + length(which(sth[sth1] == 1))
  } else {
